...
def face_trace(mesh: DLFLMesh, face: FaceKey) -> Generator[HalfEdge, None, None]:
...
def vertex_trace(mesh: DLFLMesh, vertex: VertexKey) -> Generator[HalfEdge, None, None]:
    ...
    first_half_edge = HalfEdge(
        (vertex, circular_list.next_item(mesh.face_vertices[first_face], vertex)),
        first_face,
    )
    ...
    face = next(
        key
        for key in mesh.vertex_faces[first_half_edge.vertices[1]]
    ...
    while half_edge != first_half_edge:
        ...
        if half_edge.vertices[1] == vertex:
            ...
            face = next(
                key
                for key in mesh.vertex_faces[half_edge.vertices[1]]
                if (half_edge.vertices[1], half_edge.vertices[0])
                in circular_list.pairs(mesh.face_vertices[key])
            )
            half_edge = HalfEdge((half_edge.vertices[1], half_edge.vertices[0]), face)
...
def insert_edge(
    ...
    """
    return (_insert_edge_cofacial if face_1 == face_2 else _insert_edge_non_cofacial)(
...
def _insert_edge_cofacial(
    ...
    new_face_1_vertices, new_face_2_vertices = circular_list.split_at_item(
        circular_list.circulated_to_item(mesh.face_vertices[old_face], vertex_2),
        vertex_1,
    )
...
def delete_edge(
    ...
    """
    return (_delete_edge_cofacial if face_1 == face_2 else _delete_edge_non_cofacial)(
        mesh, vertex_1, face_1, vertex_2, face_2
    )
